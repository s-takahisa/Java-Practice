...
public class Monster3 {
	...
	Monster3(String tName ,String mMame){
	...
	Monster3(String tName, String mMame, int level){
		this();
		this.trainer = tName;
		this.name = mMame;
		this.lv = level;
	...
	//---------------setWazaメソッド---------------
		...
		public void setWaza(String wnm , String dmg) {
			//バリデーションチェック
		...
		public int useWaza() {
				...
				//攻撃を変数atkDmgのインスタンス化する。
				BigDecimal atkDmg = new BigDecimal(atk);
				//わざを変数wazaDmgのインスタンス化する。
				BigDecimal wazaDmg = new BigDecimal(wazaDamage);
				//攻撃×わざのダメージを定義する。そしてintValueメソッドで小数点以下を切り捨て。
				int dmg = (atkDmg.multiply(wazaDmg)).intValue();
				return dmg;
			}
			...
			//---------------damagedメソッド---------------
			//int型の仮引数をdmgdとする。
		public int damaged(int dmgd) {
				//ダメージ減算率
				//ダメージをBigDecimalにて定義
				BigDecimal dmgRate = new BigDecimal(dmgd);
				//ダメージ減さん率120を定義
				BigDecimal dmg120 = new BigDecimal("120");
				//防御をBigDecimalで定義
				BigDecimal dmgDef = new BigDecimal(def);
				//ダメージ減算率:1 / (1+ぼうぎょ÷120) ※小数第3位切り捨て
				BigDecimal DMAGE_1 = new BigDecimal("1");
				BigDecimal dmg = DMAGE_1.divide(DMAGE_1.add(dmgDef.divide(dmg120, RoundingMode.DOWN ) ), 2, RoundingMode.DOWN ) ;
				//変数dmg2に実際にうけたダメージを計算する。
				//値渡しされたダメージ値×ダメージ減算率、int型への変換はintValueメソッドで
				int dmg2 = ( dmgRate.multiply( dmg ) ).intValue();
